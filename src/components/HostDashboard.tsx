        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <button
              ...
              className="relative inline-flex items-center justify-center rounded-xl border border-zinc-200 dark:border-slate-700 p-2.5 text-zinc-700 dark:text-slate-200 hover:bg-zinc-50 dark:bg-slate-900/50 dark:hover:bg dark:hover:bg-slate-900/50"
              title="Thông báo"
            >
                ...
                {notifications.length === 0 ? (
                  ...
                  <div className="divide-y divide-zinc-100">
                    {notifications.map((noti) => (
                      <div
                        key={noti.id}
                        className={`p-3 text-sm cursor-pointer hover:bg-zinc-50 dark:bg-slate-900/50 dark:hover:bg dark:hover:bg-slate-900/50 transition ${!noti.is_read ? 'bg-blue-50/50' : ''}`}
                        onClick={async () => {
                          if (!noti.is_read) {
                            try {
            ...
            {showForm && (
              <form onSubmit={saveExperience} className="grid gap-3 rounded-2xl border border-zinc-200 dark:border-slate-700 bg-zinc-50 dark:bg-slate-900/50 p-4 lg:grid-cols-6">
                ...
                <div className="flex items-end justify-end gap-2 lg:col-span-6">
                  ...
                  <button type="button" onClick={resetForm} className="rounded-xl border border-zinc-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-5 py-2 text-sm font-bold text-zinc-600 dark:text-slate-300 hover:bg-zinc-100 dark:hover:bg-slate-800">Hủy</button>
                </div>
              </form>
            )}
