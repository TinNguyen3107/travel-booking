...
export default function ScheduleManager({ experienceId, experienceTitle, onClose }: ScheduleManagerProps) {
  ...
  const fetchSchedules = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/schedules?experience_id=${experienceId}`);
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {schedules.map(schedule => (
                <div key={schedule.id} className="flex items-center justify-between rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
